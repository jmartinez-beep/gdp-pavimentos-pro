...
def pavement_3d_figure(selected: Dict, sclass: str, cbr: float, exploded: bool = False) -> go.Figure:
    road_length = 10.0
    road_width = 6.0
    gap = 3.0 if exploded else 0.0

    layers = []
    if float(selected.get("Carpeta_cm", 0)) > 0:
        layers.append(("Carpeta asfáltica", float(selected["Carpeta_cm"]), "#252727"))
    else:
        layers.append((str(selected.get("Superficie", "Superficie")), 2.0, "#252727"))
    layers.extend([
        ("Base granular", float(selected.get("Base_cm", 0)), "#8f8575"),
        ("Subbase granular", float(selected.get("Subbase_cm", 0)), "#9a6337"),
        (f"Subrasante {sclass}", 25.0, "#5a3824"),
    ])
    ...
    for idx, (name, thickness, color) in enumerate(reversed_layers):
        if thickness <= 0:
            continue
        z0 = z_cursor + idx * gap
        z1 = z0 + thickness
        traces.extend(_textured_box(0, road_length, 0, road_width, z0, z1, name))
        label = f"<b>{name}</b><br>{thickness:.0f} cm" if "Subrasante" not in name else f"<b>{name}</b><br>CBR {cbr:.2f}%"
        annotations.append(dict(
            x=road_length + 0.55, y=road_width * 0.78, z=(z0 + z1) / 2,
            text=label, showarrow=True, arrowhead=0, arrowsize=1,
            arrowwidth=2, arrowcolor=color, ax=55, ay=0,
            bgcolor="rgba(7,20,33,.94)", bordercolor=color, borderwidth=2,
            font=dict(size=12, color="#f4f7f8"),
            opacity=.96,
        ))
        z_cursor = z1
        top_surface_z = max(top_surface_z, z1)

    # Marcas viales de referencia: hacen que la capa superior se lea como una calzada real.
    traces.extend(_road_marking_traces(0, road_length, 0, road_width, top_surface_z + 0.12))

    # Plataforma inferior para mejorar profundidad y contraste.
    platform, platform_edges = _box_mesh(-.45, road_length+.45, -.45, road_width+.45, -2.2, -.5, "Base visual", "#0b2340")
    platform.update(opacity=.72, hoverinfo="skip", showlegend=False)
    traces = [platform, platform_edges] + traces

    fig = go.Figure(data=traces)
    fig.update_layout(
        height=690,
        paper_bgcolor="#06121d",
        plot_bgcolor="#06121d",
        margin=dict(l=0, r=0, t=58, b=0),
        title=dict(text=f"<b>Modelo 3D — estructura {selected.get('Código', '')}</b>", x=0.025, font=dict(color="white", size=18)),
        showlegend=False,
        scene=dict(
            bgcolor="#06121d",
            ...
            zaxis=dict(title="Espesor (cm)", showbackground=True, backgroundcolor="#091a27", gridcolor="#26445b", zerolinecolor="#55758d", color="#d9efff", showspikes=False),
            aspectmode="manual",
            aspectratio=dict(x=1.70, y=1.03, z=1.02),
...
def render_rotating_3d(fig: go.Figure, key: str, height: int = 690, auto_rotate: bool = True) -> None:
    """Render Plotly 3D with slow auto-rotation and interactive pause/resume controls."""
    ...
    <style>
      ...
      .gdp-controls{{position:absolute;left:16px;bottom:14px;z-index:20;display:flex;align-items:center;gap:8px;background:rgba(3,13,23,.88);border:1px solid #315878;border-radius:10px;padding:7px 10px;box-shadow:0 8px 22px rgba(0,0,0,.35);color:#eaf5ff;font-size:13px}}
      .gdp-btn{{border:1px solid #2e6ea3;background:#0a2b49;color:#fff;border-radius:7px;padding:6px 10px;font-weight:700;cursor:pointer}}
      .gdp-btn:hover{{background:#0d69ca}}
      .gdp-state{{color:#6ed7ff;min-width:72px;font-weight:700}}
      .gdp-hint{{position:absolute;right:16px;bottom:16px;z-index:19;background:rgba(3,13,23,.78);border:1px solid #27475f;color:#c7d8e7;border-radius:8px;padding:7px 10px;font-size:12px}}
    </style>
    <div class="gdp-wrap">
      <div class="gdp-plot">{plot_html}</div>
      <div class="gdp-controls">
        <button id="pause_{key}" class="gdp-btn">⏸ Pausar</button>
        <button id="resume_{key}" class="gdp-btn">▶ Reanudar</button>
        <span id="state_{key}" class="gdp-state">Rotando…</span>
      </div>
      <div class="gdp-hint">Arrastre: orbitar · Scroll: zoom · Botones: pausar/reanudar</div>
    </div>
    <script>
    (function(){{
      const gd = document.getElementById('{div_id}');
      const pauseBtn = document.getElementById('pause_{key}');
      const resumeBtn = document.getElementById('resume_{key}');
      ...
      const speed = 0.0030;
      function syncState(){{
        state.textContent = running ? 'Rotando…' : 'Pausado';
        state.style.color = running ? '#6ed7ff' : '#ffc857';
      }}
      pauseBtn.onclick = () => {{running=false; syncState();}};
      resumeBtn.onclick = () => {{running=true; syncState();}};
      syncState();
      gd.on('plotly_relayout', function(ev){{
        ...
        if(cam && cam.eye){{
          ...
          if(Number.isFinite(e.x) && Number.isFinite(e.y)){{
            radius = Math.max(0.65, Math.hypot(e.x,e.y));
            angle = Math.atan2(e.y,e.x);
          }}
          if(Number.isFinite(e.z)) eyeZ=e.z;
        }}
      }});
      function tick(){{
        if(!running || document.hidden) return;
        angle += speed;
        const eye = {{x:radius*Math.cos(angle), y:radius*Math.sin(angle), z:eyeZ}};
        internalUpdate=true;
        Plotly.relayout(gd, {{'scene.camera.eye':eye}})
          .then(()=>{{internalUpdate=false;}})
          .catch(()=>{{internalUpdate=false;}});
      }}
      setInterval(tick, 80);
    }})();
    </script>
...
def deterioration_3d_figure(selected: Dict, sclass: str, cbr: float, state: Dict, visible_pathologies: List[str]) -> go.Figure:
    ...
    asphalt_cm = float(selected.get("Carpeta_cm", 0) or 0)
    if asphalt_cm <= 0:
        asphalt_cm = 2.0
