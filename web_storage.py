...
DATABASE_URL = (os.getenv("DATABASE_URL") or os.getenv("GDP_DATABASE_URL") or "").strip()
...
def _connect():
    if DATABASE_URL:
        ...
        return psycopg.connect(DATABASE_URL, row_factory=dict_row, connect_timeout=20)
...
def create_user(username: str, password: str, display_name: str = "") -> tuple[bool, str]:
    ...
    try:
        with _connect() as con:
            con.execute(
                _sql("INSERT INTO users(username,password_hash,display_name) VALUES(?,?,?)"),
...
def authenticate(username: str, password: str) -> dict[str, Any] | None:
    query = (
        ...
        if DATABASE_URL
        else "SELECT id,username,password_hash,display_name FROM users WHERE username=?"
    )
    with _connect() as con:
        row = con.execute(_sql(query), (username.strip(),)).fetchone()
...
def load_project(user_id: int, project_id: int) -> dict[str, Any] | None:
    with _connect() as con:
        row = con.execute(
            _sql("SELECT payload_json FROM projects WHERE id=? AND user_id=?"),
            (project_id, user_id),
        ).fetchone()
    if not row:
        return None
...
def delete_project(user_id: int, project_id: int) -> None:
    with _connect() as con:
        con.execute(
            _sql("DELETE FROM projects WHERE id=? AND user_id=?"),
            (project_id, user_id),
        )
